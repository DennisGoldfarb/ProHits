...
//echo "\gene_UniProt_arr_file=$gene_UniProt_arr_file<br>";
    ...
    if(!_is_file($gene_UniProt_arr_file)){
      ...
      $SQL = "SELECT UPDATE_TIME
              ...
              WHERE  TABLE_SCHEMA = '".PROHITS_PROTEINS_DB."'
              AND TABLE_NAME = 'Protein_mapping'";
      $tmp_arr = $proteinDB->fetch($SQL);
      $Protein_mapping_update_time = $tmp_arr['UPDATE_TIME'];
      $gene_UniProt_arr_file_mtime = @date("Y-m-d H:i:s", filemtime($gene_UniProt_arr_file));
      if(strcmp($Protein_mapping_update_time, $gene_UniProt_arr_file_mtime) > 0){
        $create_new_gene_UniProt_file_flag = 1;
      }
    }
  ...
  //------------------------------------------------------------------------
    add_uniq_pep();
    $gene_UniProt_handle = fopen($gene_UniProt_arr_file, 'a');
    if($gene_UniProt_handle){
      foreach($NEW_gene_UniProt_arr as $key => $value){
        fwrite($gene_UniProt_handle,$key.",".$value."\n");
      }
    }
...
?>
    <tr  bgcolor='<?php echo $bgcolor;?>' onmousedown="highlightTR(this, 'click', '#CCFFCC', '<?php echo $bgcolor;?>');">
      ...
      if($saint_records[$i]['UserID'] == $USER->ID and !isset($running_saint_arr[$saint_records[$i]['ID']])){
        echo "<a href=\"javascript: saint_delete('".$saint_records[$i]['ID']."')\"  title='delete'>
        <img src='./images/icon_purge.gif' border=0></a>\n";
        if(!$saint_records[$i]['ParentSaintID'] or $saint_records[$i]['Status'] != 'Finished'){
          echo "<a href=\"javascript: run_saint('".$saint_records[$i]['ID']."','".$saint_records[$i]['UserOptions']."' )\"  title='re-run SAINT'>
      ...
      ?>&nbsp;&nbsp;
      <a href="javascript: saint_input_files('<?php echo $saint_records[$i]['ID'];?>')"  title='SAINT input files'>
      ...
      <?php if($saint_records[$i]['Status'] == 'Finished'){
          ...
          echo "<a href=\"javascript: generate_report('".$saint_records[$i]['ID']."','$is_uploaded','$SAINT_v')\"  title='SAINT results'>";
          echo "<img src=\"./images/icon_report.gif\" border=0>";
          echo "</a>&nbsp;&nbsp;";
          echo "<a href=\"javascript: export_saint_results('".$saint_records[$i]['ID']."')\"  title='download SAINT results'>";
...
if($tmp_counter > 100){
  exit;
    ...
    $accID = $tmp_arr[1];
    $geneName = $tmp_arr[2];
    if($has_log_file && strpos($accID, 'DECOY') === false){
