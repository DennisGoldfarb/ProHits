...
//echo "$buffer<br>";
  ...
  foreach($lines_export as $line_export){
    ...
    if(!in_array($inf_arr['Vector'], $Vector_arr)){
...
<?php }?>    
  ...
  <?php if(($USER->Type == 'Admin' || $export_to_Prohits_permission) && defined('PROHITS_WEB')){?>    
...
if(!$line_check_box_checked){
  ...
  fwrite($html_handle, $html_str);
     ...
     ?> 
        <a class=sTitle title='check the box then click GO button to remove this cell'>
