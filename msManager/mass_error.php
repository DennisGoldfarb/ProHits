...
$errorList = array();       // array of experimental errors
$massList = array();        // array of experimental Mr values
$urlParams = array();       // keys are URL parameter names in lower case, values are names in original case(a)
  
require("../common/site_permission.inc.php");

// get parameters
if(!($fileIn = $file)) $error_message = "Error: no file name";
if(!($hitNum = $hit)) $error_message = "Error: no hit number";
...
// $fileIn contains list of experimental masses
// $hitNum contains list of experimental errors
if(preg_match("/^massList:(.*)/i", $fileIn, $matches)){
  $massList = explode(',',$matches[1]);
  if(preg_match("/^errorList:(.*)/i", $hitNum, $matches2)){
    $errorList = explode(',',$matches2[1]);
  }  
} else {
  $error_message = "Error:arguments have obsolete format";
}
...
// RMS error in ppm
  ...
  if(count($errorList)){
    for($i=0; $i<count($errorList); $i++){
       $RMS_error += pow(($errorList[$i] * 1000000 / $massList[$i]), 2);
    }
    $RMS_error = floor(pow(($RMS_error / count($errorList)),0.5));
  }
...
// unit conversion
  if($units != "Da") {
    for($i=0; $i<count($errorList); $i++){
      if($units == "%") {
        $errorList[$i] = $errorList[$i] * 100 / $massList[$i];
      }elseif($units == "ppm") {
        $errorList[$i] = $errorList[$i] * 1000000 / $massList[$i];
      }elseif($units == "mmu") {
        $errorList[$i] = $errorList[$i] * 1000;
      }else{
        $units = "Da";
      }
    }
...
// find max and min error values
  if($count = count($errorList)){
    $errorMin = 99999999;   
    $errorMax = -99999999;
    $i = 0;
    while($i < $count){
      if($errorList[$i] > $errorMax){
        $errorMax = $errorList[$i];
      }
      if($errorList[$i] < $errorMin){
        $errorMin = $errorList[$i];
      }
      $i++;
    }
...
// find max and min mass values
  if($count = count($massList)){
    $massMin = 99999999;   
    $massMax = -1;
    $i = 0;
    while($i < $count){
      if($massList[$i] > $massMax){
        $massMax = $massList[$i];
      }
      if($massList[$i] < $massMin){
        $massMin = $massList[$i];
      }
      $i++;
    }
...
// draw data points
  for ($i = 0; $i<count($errorList); $i++) {
    $thisY = -($errorList[$i]-$yFirstTick)*$yScale+$yOffset;
    $thisX = ($massList[$i]-$xFirstTick)*$xScale+$xOffset;
